...
public class VectorClock implements Version, Serializable {
    ...
    public VectorClock(byte[] bytes, int offset) {
        ...
        //System.out.println("BYTES: " + this.timestamp);
    }
    ...
    public static Version createNew(DataInputStream inputStream) {
        try {
            ...
            try {
                Constructor constructor = Constants.getVersionType().getConstructor(vectorClockBytes.getClass());
    ...
    public Version merge(Version clock) {
        ...
        for(Map.Entry<Short, Long> entry: ((VectorClock) clock).versionMap.entrySet()) {
    ...
    public void updateVersion(String nodeId, long newVersion, long currentTimeMillis) {
        ...
        synchronized (versionMap){
            versionMap.put(Short.parseShort(nodeId), newVersion);
            if(versionMap.size() >= MAX_NUMBER_OF_VERSIONS) {
                throw new IllegalStateException("Vector clock is full!");
            }
        }
    ...
    public Map<String, Long> getVersions() {
        Map<String, Long> output = new TreeMap<>();
        versionMap.forEach((k, v) -> output.put(String.valueOf(k), v));
        return output;
    }
