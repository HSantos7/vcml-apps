...
public class VoldemortAdminTool {
    ...
    public static void executeSetMetadataPair(Integer nodeId,
        ...
        List<Integer> nodeIds = Lists.newArrayList();
        Version updatedClusterVersion = null;
        Version updatedStoresVersion = null;
        if(nodeId < 0) {
            for(Node node: adminClient.getAdminClientCluster().getNodes()) {
                ...
                if(updatedClusterVersion == null && updatedStoresVersion == null) {
                    updatedClusterVersion = adminClient.metadataMgmtOps.getRemoteMetadata(node.getId(),
                                                                                                        clusterKey)
                                                                                     .getVersion();

                    updatedStoresVersion = adminClient.metadataMgmtOps.getRemoteMetadata(node.getId(),
                                                                                                       storesKey)
                                                                                    .getVersion();
                } else {
            ...
            updatedClusterVersion = (adminClient.metadataMgmtOps.getRemoteMetadata(nodeId,
                                                                                                 clusterKey)
                                                                              .getVersion()).incremented(nodeId,
                                                                                                         System.currentTimeMillis());
            updatedStoresVersion = (adminClient.metadataMgmtOps.getRemoteMetadata(nodeId,
                                                                                                storesKey)
                                                                             .getVersion()).incremented(nodeId,
                                                                                                        System.currentTimeMillis());
        ...
        if(nodeId < 0) {
            for(Node node: adminClient.getAdminClientCluster().getNodes()) {
                ...
                if(updatedVersion == null) {
                    updatedVersion = adminClient.metadataMgmtOps.getRemoteMetadata(node.getId(),
                                                                                                 key)
                                                                              .getVersion();
                } else {
                    updatedVersion = updatedVersion.merge(adminClient.metadataMgmtOps.getRemoteMetadata(node.getId(),
                                                                                                                      key)
                                                                                                   .getVersion());
                }
            }
            ...
            Versioned<String> currentValue = adminClient.metadataMgmtOps.getRemoteMetadata(nodeId,
                                                                                           key);
            updatedVersion = (currentValue.getVersion()).incremented(nodeId,
                                                                                   System.currentTimeMillis());
            nodeIds.add(nodeId);
        }
