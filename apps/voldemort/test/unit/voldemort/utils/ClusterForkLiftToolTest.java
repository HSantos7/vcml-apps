...
public class ClusterForkLiftToolTest {
    ...
    public void testPrimaryResolvingForkLift() throws Exception {
        ...
        srcAdminClient.storeOps.putNodeKeyValue(PRIMARY_RESOLVING_STORE_NAME,
                                                new NodeValue<voldemort.consistency.utils.ByteArray, byte[]>(masterNode,
                                                                                 new voldemort.consistency.utils.ByteArray(conflictKey.getBytes("UTF-8")),
    ...
    public void testGloballyResolvingForkLift() throws Exception {
        ...
        srcAdminClient.storeOps.putNodeKeyValue(GLOBALLY_RESOLVING_STORE_NAME,
                                                new NodeValue<voldemort.consistency.utils.ByteArray, byte[]>(nodeList.get(0),
                                                                                 new voldemort.consistency.utils.ByteArray(conflictKey.getBytes("UTF-8")),
                                                                                 new Versioned<byte[]>("losing value".getBytes("UTF-8"),
                                                                                                       losingClock)));
        srcAdminClient.storeOps.putNodeKeyValue(GLOBALLY_RESOLVING_STORE_NAME,
                                                new NodeValue<voldemort.consistency.utils.ByteArray, byte[]>(nodeList.get(1),
                                                                                 new voldemort.consistency.utils.ByteArray(conflictKey.getBytes("UTF-8")),
                                                                                 new Versioned<byte[]>("winning value".getBytes("UTF-8"),
                                                                                                       winningClock)));
    ...
    public void testNoresolutionForkLift() throws Exception {
        ...
        for(Node node: dstAdminClient.getAdminClientCluster().getNodes()) {
            ...
            Iterator<Pair<voldemort.consistency.utils.ByteArray, Versioned<byte[]>>> entryItr = dstAdminClient.bulkFetchOps.fetchEntries(node.getId(),
            ...
            while(entryItr.hasNext()) {
                Pair<voldemort.consistency.utils.ByteArray, Versioned<byte[]>> record = entryItr.next();
