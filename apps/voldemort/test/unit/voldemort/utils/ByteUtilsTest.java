...
public class ByteUtilsTest extends TestCase {
    ...
    public void testCat() {
        assertTrue("Concatenation of empty arrays is not empty",
                   Arrays.equals(new byte[0], voldemort.consistency.utils.ByteUtils.cat(new byte[0], new byte[0])));
        assertTrue("Concatenation of no arrays is not empty",
                   Arrays.equals(new byte[0], voldemort.consistency.utils.ByteUtils.cat()));
        assertTrue("Concatenation of arrays incorrect.",
                   Arrays.equals("abcdefg".getBytes(), voldemort.consistency.utils.ByteUtils.cat("ab".getBytes(),
                                                                     "".getBytes(),
                                                                     "cdef".getBytes(),
                                                                     "g".getBytes())));
    }
    ...
    public void testCopy() {
        assertTrue("Copy of no bytes is not empty array.",
                   Arrays.equals(new byte[0], voldemort.consistency.utils.ByteUtils.copy("hello".getBytes(), 0, 0)));
        assertTrue("Copy of negetive bytes is not empty array.",
                   Arrays.equals(new byte[0], voldemort.consistency.utils.ByteUtils.copy("hello".getBytes(), 2, 0)));
        assertTrue("Bytes copy incorrectly",
                   Arrays.equals("hell".getBytes(), voldemort.consistency.utils.ByteUtils.copy("hello".getBytes(), 0, 4)));
    }

    public void testReadWriteShort() {
        byte[] bytes = new byte[4];
        voldemort.consistency.utils.ByteUtils.writeShort(bytes, (short) 5, 0);
        assertEquals("Read value not equal to written value.",
                     (short) 5,
                     voldemort.consistency.utils.ByteUtils.readShort(bytes, 0));
        voldemort.consistency.utils.ByteUtils.writeShort(bytes, (short) 500, 2);
        assertEquals("Read value not equal to written value.",
                     (short) 500,
                     voldemort.consistency.utils.ByteUtils.readShort(bytes, 2));
    }

    public void testReadWriteLong() {
        byte[] bytes = new byte[8];
        voldemort.consistency.utils.ByteUtils.writeLong(bytes, 5, 0);
        assertEquals("Read value not equal to written value.", 5, voldemort.consistency.utils.ByteUtils.readLong(bytes, 0));
        long value = System.currentTimeMillis();
        voldemort.consistency.utils.ByteUtils.writeLong(bytes, value, 0);
        assertEquals("Read value not equal to written value.", value, voldemort.consistency.utils.ByteUtils.readLong(bytes, 0));
    }

    public void testReadWriteInt() {
        byte[] bytes = new byte[8];
        voldemort.consistency.utils.ByteUtils.writeInt(bytes, 5, 0);
        assertEquals("Read value not equal to written value.", 5, voldemort.consistency.utils.ByteUtils.readInt(bytes, 0));
        int value = (int) System.currentTimeMillis();
        voldemort.consistency.utils.ByteUtils.writeInt(bytes, value, 0);
        assertEquals("Read value not equal to written value.", value, voldemort.consistency.utils.ByteUtils.readInt(bytes, 0));
    }
    ...
    public void testReadUnsignedInt() {
        ...
        bytes[2] = (byte) 0xFF;
        bytes[3] = (byte) 0xFF;
        assertEquals(0xFFFFFFFF, voldemort.consistency.utils.ByteUtils.readInt(bytes, 0));
        bytes[0] = (byte) 0x01;
        bytes[1] = (byte) 0x23;
        bytes[2] = (byte) 0x45;
        bytes[3] = (byte) 0x67;
        assertEquals(0x01234567, voldemort.consistency.utils.ByteUtils.readInt(bytes, 0));
    }

    public void testReadWriteBytes() {
        byte[] bytes = new byte[8];
        voldemort.consistency.utils.ByteUtils.writeBytes(bytes, 5, 0, 8);
        assertEquals("Read value not equal to written value.", 5, voldemort.consistency.utils.ByteUtils.readBytes(bytes, 0, 8));
        long value = System.currentTimeMillis();
        voldemort.consistency.utils.ByteUtils.writeBytes(bytes, value, 0, 8);
        assertEquals("Read value not equal to written value.",
                     value,
                     voldemort.consistency.utils.ByteUtils.readBytes(bytes, 0, 8));
        bytes = new byte[24];
        voldemort.consistency.utils.ByteUtils.writeBytes(bytes, value, 8, 8);
        assertEquals("Read value not equal to written value.",
                     value,
                     voldemort.consistency.utils.ByteUtils.readBytes(bytes, 8, 8));
    }

    public void testGetNumberOfRequiredBytes() {
        assertEquals(1, voldemort.consistency.utils.ByteUtils.numberOfBytesRequired(0));
        assertEquals(1, voldemort.consistency.utils.ByteUtils.numberOfBytesRequired(1));
        assertEquals(2, voldemort.consistency.utils.ByteUtils.numberOfBytesRequired(257));
        assertEquals(3, voldemort.consistency.utils.ByteUtils.numberOfBytesRequired(3 * Short.MAX_VALUE));

    }

    public void testToString() {
        assertEquals("00", voldemort.consistency.utils.ByteUtils.toHexString(new byte[] { 0 }));
        assertEquals("010203", voldemort.consistency.utils.ByteUtils.toHexString(new byte[] { 1, 2, 3 }));
        assertEquals("afadae",
                     voldemort.consistency.utils.ByteUtils.toHexString(new byte[] { (byte) 0xaf, (byte) 0xad, (byte) 0xae }));
        assertEquals("afadae",
                     voldemort.consistency.utils.ByteUtils.toHexString(new byte[] { (byte) 0xaf, (byte) 0xad, (byte) 0xae }));
        assertEquals("",
                     voldemort.consistency.utils.ByteUtils.toHexString(new byte[] { (byte) 0, (byte) 1, (byte) 2, (byte) 3,
        ...
        assertEquals("00000000", voldemort.consistency.utils.ByteUtils.toBinaryString(new byte[] { 0 }));
        assertEquals("00000001" + "00000010" + "00000011",
                     voldemort.consistency.utils.ByteUtils.toBinaryString(new byte[] { 1, 2, 3 }));
        assertEquals("10101111" + "10101101" + "10101110",
                     voldemort.consistency.utils.ByteUtils.toBinaryString(new byte[] { (byte) 0xaf, (byte) 0xad, (byte) 0xae }));

    }

    public void specificFromHexStringTest(byte[] lhsBytes, String rhsString) {
        try {
            byte[] rhsBytes = voldemort.consistency.utils.ByteUtils.fromHexString(rhsString);
            int offset = 0;
            for(byte b: lhsBytes) {
    ...
    public void testNthByte() {
        assertEquals((byte) 0xFF, voldemort.consistency.utils.ByteUtils.readNthByte(Long.MAX_VALUE, 0));
        assertEquals((byte) 0xFF, voldemort.consistency.utils.ByteUtils.readNthByte(Long.MAX_VALUE, 1));
    ...
    public void testNthByteRead(String theByte, String theLong, int nth) {
        ...
        assertEquals(asByte, voldemort.consistency.utils.ByteUtils.readNthByte(asLong, nth));
    }

    public void testMd5() {
        String test = "alskdjflsajflksdjldfsdf";
        MessageDigest digest = voldemort.consistency.utils.ByteUtils.getDigest("MD5");
        assertEquals(0,
                     voldemort.consistency.utils.ByteUtils.compare(voldemort.consistency.utils.ByteUtils.md5(test.getBytes()),
                                       digest.digest(test.getBytes())));
    }

    public void testSha1() {
        voldemort.consistency.utils.ByteUtils.sha1("hello".getBytes());
    }

    public void testByteComparison() {
        assertEquals(0, voldemort.consistency.utils.ByteUtils.compare(new byte[] {}, new byte[] {}));
        assertEquals(1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 1 }, new byte[] {}));
        assertEquals(-1, voldemort.consistency.utils.ByteUtils.compare(new byte[] {}, new byte[] { 1 }));
        assertEquals(0, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 0, 1, 2 }, new byte[] { 0, 1, 2 }));
        assertEquals(-1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 0, 1, 1 }, new byte[] { 0, 1, 2 }));
        assertEquals(-1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 0, 1, 1 }, new byte[] { 0, 1, 1, 2 }));
        assertEquals(1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 0, 1, 2 }, new byte[] { 0, 1, 1, 1 }));
        assertEquals(-1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 0, 1, 2 }, new byte[] { 4, 1, 1, 1 }));
        assertEquals(1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 0, 1, 4 }, new byte[] { 0, 1, 1, 1 }));
        assertEquals(-1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 1 }, new byte[] { -1 }));
        assertEquals(0, voldemort.consistency.utils.ByteUtils.compare(new byte[] { -1 }, new byte[] { -1 }));
        assertEquals(1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { -1 }, new byte[] { -4 }));
        assertEquals(-1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { -4 }, new byte[] { -1 }));

        // Tests byte comparison with sub-sequence
        assertEquals(1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 0 }, new byte[] { 0 }, 0, 0));
        assertEquals(0, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 0 }, new byte[] { 0 }, 0, 1));
        try {
            assertEquals(0, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 0 }, new byte[] { 0 }, 0, 2));
            fail("Should have thrown an exception");
        } catch(IllegalArgumentException e) {}
        assertEquals(-1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 2 }, new byte[] { 0, 1, 2, 3 }, 3, 4));
        assertEquals(0, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 3 }, new byte[] { 0, 1, 2, 3 }, 3, 4));
        assertEquals(1, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 5 }, new byte[] { 0, 1, 2, 3 }, 3, 4));

        assertEquals(0, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 2, 3 }, new byte[] { 0, 1, 2, 3 }, 2, 4));
        try {
            assertEquals(0, voldemort.consistency.utils.ByteUtils.compare(new byte[] { 2, 3 }, new byte[] { 0, 1, 2, 3 }, 2, 5));
            fail("Should have thrown an exception");
        } catch(IllegalArgumentException e) {}
