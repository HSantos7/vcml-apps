...
package edu.msu.cse.cops.server.consistency.utils;
...
public class Utils {
    ...
    private static final Logger logger = Logger.getLogger(edu.msu.cse.cops.server.consistency.utils.Utils.class);
    ...
    public static void symlink(String filePath, String symLinkPath) {
        ...
        if (!file.exists()) {
            throw new ConsistencyException("File " + filePath + " does not exist");
        }
        ...
        if (symLink.exists()) {
            try {
                ...
                throw new ConsistencyException("Got an IOException while trying to read a symlink.", e);
            }
        }
        ...
        if (returnCode < 0)
            throw new ConsistencyException("Unable to create symbolic link for " + filePath +
    ...
    public static void move(File source, File dest) {
        if(!source.exists())
            throw new ConsistencyException("File " + source.toString() + " does not exist.");
        Utils.rm(dest);
        boolean succeeded = source.renameTo(dest);
        if(!succeeded)
            throw new ConsistencyException("Rename of " + source + " to " + dest + " failed.");
    }
    ...
    public static void mkdirs(File newDir) {
        if(newDir.exists()) {
            if(!newDir.canWrite() || !newDir.canRead())
                throw new ConsistencyException("Unable to access directory "
                                             + newDir.getAbsolutePath());
        } else {
            if(!newDir.mkdirs())
                throw new ConsistencyException("Unable to create directory "
                                             + newDir.getAbsolutePath());
        }
    }
     ...
     * return the argument.
     * 
     * Useful for assignment as in this.thing = BitsUtils.notNull(thing);
     * 
    ...
    /**
     * Computes sum of a {@link Long} list
     * 
     * @param list
    ...
    /**
     * Compute the average of a {@link Long} list
     * 
     * @param list
     * @return
     */
    ...
    /**
     * Compute the sum of a {@link Double} list
     * 
     * @param list
     * @return
     */
    ...
    /**
     * Compute the average of a {@link Double} list
     * 
     * @param list
     * @return
     */
     ...
     */
    public static List<String> nodeListToNodeIdList(List<Node> nodes) {
